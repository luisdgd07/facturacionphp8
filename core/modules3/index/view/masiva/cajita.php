                ...
                <?php if (($sell->getVenta()->getCliente()->tipo_doc=="SIN NOMBRE" or $sell->getVenta()->getCliente()->tipo_doc=="CEDULA DE EXTRANJERO")  &(($sell->getVenta()->total))>$sell->getventa()->cantidaconfigmasiva): ?>
                    <td> <?php echo $sell->venta_id; ?></td> 
                      ...
                      <td><?php if ($sell->getProducto()->impuesto=="10"): ?>
                        <b style="color: blue;"><?php echo $sell->getProducto()->impuesto ?>% <?php   (($sell->getVenta()->total)/11);$total10imp=(($sell->getVenta()->total)/11);?></b>
                      <?php else: ?>
                        <?php if ($sell->getProducto()->impuesto=="5"): ?>
                        <b style="color: green;"><?php echo $sell->getProducto()->impuesto ?>% <?php   (($sell->getVenta()->total)/21);$total5imp=(($sell->getVenta()->total)/21);?></b>
                        <?php else: ?>
                        <?php if (
						
						
						$sell->getProducto()->impuesto=="0"): ?>
                        <b style="color: red;"><?php echo $sell->getProducto()->impuesto ?>% <?php   ($sell->getVenta()->total);$tota0imp=($sell->getVenta()->total);?></b>
                      <?php endif ?>
                      <?php endif ?>
                      <?php endif ?></td>
                      <td><?php if ($sell->getProducto()->impuesto=="10"): ?>
                      ...
                      <?php endif ?></td>
                      <td><?php if ($sell->getProducto()->impuesto=="10"): ?>
                        <b style="color: blue;"><?php  echo number_format(((($sell->getVenta()->total/$sell->q)*$sell->q)/1.1),4,',','.');$totaliva10=((($sell->getVenta()->total/$sell->q)*$sell->q)/1.1)?></b>
                      <?php else: ?>
                        <?php if ($sell->getProducto()->impuesto=="5"): ?>
                        <b style="color: green;"><?php  echo number_format(((($sell->getVenta()->total/$sell->q)*$sell->q)/1.05),4,',','.');$totaliva5=((($sell->getVenta()->total/$sell->q)*$sell->q)/1.05)?></b>
                      <?php else: ?>
                        <?php if ($sell->getProducto()->impuesto=="0"): ?>
                        <b style="color: red;"><?php  echo number_format((($sell->getVenta()->total/$sell->q)*$sell->q),4,',','.');$totaliva0=(($sell->getVenta()->total/$sell->q)*$sell->q)?></b>
                      <?php endif ?>
                      <?php endif ?>
                      <?php endif ?></td>
						   ...
						   <td> <?php echo $sell->getVenta()->cambio2 ?></td>
					  
                      <!-- <td><input type="text" name="num1" id="serie"></td> -->
                      <?php if ($sell->masiva==0): ?>
                      <td width="140px">
                        <a href="index.php?view=masiva1&id_sucursal=<?php echo $_GET["id_sucursal"]; ?>&id_proceso=<?php echo $sell->id_proceso; ?>" class="btn btn-success">Registrar masiva </a><div class="modal fade" id="registromasivo-<?php echo $sell->id_proceso; ?>">
                          <div class="modal-dialog">
                              <div class="modal-content">
                                  <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span></button>
                                    <h4 class="modal-title"><i  style="color: blue;"></i><b style="color: black;"> REGISTRO DE VENTA MASIVA</b></h4>
                                  </div>
...
?>
                      ...
                      <?php else: ?>
                        <td width="140"><a href="" class="btn btn-info"></a> <!-- <a href="ticket2.php?id_venta=<?php echo $sell->getVenta()->id_venta;?>" class="btn btn-warning" target="_BLANCK">Factura</a> --> <a href="imprisionticketmasiva.php?id_venta=<?php echo $sell->getVenta()->id_venta;?>" class="btn btn-warning" target="_BLANCK">Imprimir</a>
                        </td>
                    <?php endif ?>
                    <?php endif ?>
