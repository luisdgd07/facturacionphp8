...
if ($_GET['created'] == 'true') {
    ...
    if ($contrato_cliente_file !== '') {
        $update_fields[] = "`contrato_cliente` = '$contrato_cliente_file'";
    }

    if ($archivo_identificacion !== '') {
        $update_fields[] = "`form_identificacion` = '$archivo_identificacion'";
    }

    if ($archivo_constitucion !== '') {
        $update_fields[] = "`escri_constitucion` = '$archivo_constitucion'";
    }

    if ($archivo_cedula_iden !== '') {
        $update_fields[] = "`cedula_iden` = '$archivo_cedula_iden'";
    }

    if ($archivo_cedula_tribu !== '') {
        $update_fields[] = "`cedula_tribu` = '$archivo_cedula_tribu'";
    }

    if ($archivo_declaracion_iva !== '') {
        $update_fields[] = "`declaracion_iva` = '$archivo_declaracion_iva'";
    }

    if ($archivo_balance_gen !== '') {
        $update_fields[] = "`balance_gen` = '$archivo_balance_gen'";
    }

    if ($archivo_form_ire !== '') {
        $update_fields[] = "`form_ire` = '$archivo_form_ire'";
    }

    if ($archivo_factura_ser !== '') {
        $update_fields[] = "`factura_ser` = '$archivo_factura_ser'";
    }

    if ($archivo_cert_cumpli !== '') {
        $update_fields[] = "`cert_cumpli` = '$archivo_cert_cumpli'";
    }

    if ($archivo_decla_jur !== '') {
        $update_fields[] = "`decla_jur` = '$archivo_decla_jur'";
    }

    if ($archivo_consulta_lista_peps !== '') {
        $update_fields[] = "`consulta_lista_peps` = '$archivo_consulta_lista_peps'";
    }

    if ($archivo_consulta_lista_ofac !== '') {
        $update_fields[] = "`consulta_list_ofac` = '$archivo_consulta_lista_ofac'";
    }

    if ($archivo_matriz_riesgo !== '') {
        $update_fields[] = "`matriz_riesgo` = '$archivo_matriz_riesgo'";
    }

    if ($archivo_informconf !== '') {
        $update_fields[] = "`informconf` = '$archivo_informconf'";
    }

    if ($archivo_reg_ins_seprelad !== '') {
        $update_fields[] = "`reg_ins_seprelad` = '$archivo_reg_ins_seprelad'";
    }

    if ($total_doc_presentar !== '') {
        $update_fields[] = "`total_doc_presentar` = '$total_doc_presentar'";
    }

    if ($total_doc_presentado !== '') {
        $update_fields[] = "`total_doc_presentado` = '$total_doc_presentado'";
    }

    if ($realizado_por !== '') {
        $update_fields[] = "`realizado_por` = '$realizado_por'";
    }

    // Si hay campos que actualizar, crea la consulta
    if (!empty($update_fields)) {
        $fields_to_update = implode(", ", $update_fields);
...
header('Location: ./index.php?view=cliente&id_sucursal=22&success=true');
