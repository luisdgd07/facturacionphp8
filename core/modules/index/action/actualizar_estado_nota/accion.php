...
foreach ($operations as $op) {
    ...
    $a = $_GET["id_sucursal"];
    $j = AccionData::getByName("salida")->id_accion;
    $sell->accion_id = $j;
    $sell->sucursal_id = $a;
    // $sell->fecha = date('Y-d-m');
    ...
    $m = AccionData::getByName("salida")->id_accion;
    $stc = $op->q;
    $opd->stock_trans = $stc;
    $opd->accion_id = $m;
    $opd->venta_id = $s[1];
...
Core::redir("index.php?view=nota_de_credito&id_sucursal=" . $_GET["id_sucursal"]);
