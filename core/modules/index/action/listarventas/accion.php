<?php
$response = [];
$ventas = VentaData::buscarVentas($_GET['sucursal'], $_GET['offset'], $_GET['busqueda'], $_GET['desde'], $_GET['hasta'], $_GET['cliente']);
$paginas = VentaData::buscarVentasPaginacion($_GET['sucursal'], $_GET['offset'], $_GET['busqueda'], $_GET['desde'], $_GET['hasta'], $_GET['cliente']);
$tipoInsumo = ProductoData::verinsumo($_GET['sucursal']);
$insumoId = $tipoInsumo && $tipoInsumo->ID_TIPO_PROD ? $tipoInsumo->ID_TIPO_PROD : 0;
$int = 0;
$sucursal = SuccursalData::VerId($_GET['sucursal']);
foreach ($ventas as $venta) {
    $int++;
    $cliente = ClienteData::getById($venta->cliente_id);

    $clienteNombre = $cliente->tipo_doc == "SIN NOMBRE" ?
        $cliente->tipo_doc : $cliente->nombre . " " .
        $cliente->apellido;


    // $moneda = $venta->VerTipoModena()->simbolo == "₲" ? 'PYG' :  'USD';
    $moneda = 'USD';
    $operations = OperationData::getAllProductsByVenta($venta->id_venta);
    $productos = array();
    foreach ($operations as $operation) {
        $fila = $operation->is_sqlserver ? "id_sqlserver" : "id_producto";
        $product = ProductoData::getById($operation->producto_id, $fila);
        $precio = floatval($operation->precio);
        $tipo = TipoProductoData::VerId($product->ID_TIPO_PROD);
        if ($product->impuesto == 10) {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "10";
        } else if ($product->impuesto == 30) {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "30";
        } else if ($product->impuesto == 5) {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "5";
        } else if ($product->impuesto == 0) {
            $precioUnitario = $precio;
            $ivaTipo = 3;
            $ivaBase = "0";
            $iva = "0";
        } else {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "10";
        }
        $array = [
            "precioUnitario" => $precioUnitario,
            "codigo" => $product->codigo,
            "descripcion" => $product->nombre,
            "observacion" => "",
            "tipo" => isset($tipo->TIPO_PRODUCTO) ? $tipo->TIPO_PRODUCTO : null,
            "unidadMedida" => $product->presentacion != null ? UnidadesData::getById($product->presentacion)->nombre : null,
            "cantidad" => $operation->q,
            "cambio" => "0",
            "ivaTipo" => $ivaTipo,
            "ivaBase" => $ivaBase,
            "iva" => $iva,
            "lote" => "",
            "vencimiento" => "",
            "numeroSerie" => "",
            "numeroPedido" => ""
        ];
        array_push($productos, $array);
    }

    $vencimiento = "";
    $cuotas = null;
    if ($venta->metodopago == "Credito") {
        $credito = CreditoData::getByVentaId($venta->id_venta);

        $cuotas = isset($credito->cuotas) ? $credito->cuotas : 1;
        $vencimiento = isset($credito->vencimiento) ? $credito->vencimiento : $venta->fechapago;
    }
    $tipoop = $venta->getCliente()->tipo_operacion;

    $tipoCliente = 1;
    $esContribuyente = 1;
    // if ($venta->getCliente()->tipo_doc == "RUC") {
    //     $tipoCliente = 1;
    // } else if ($venta->getCliente()->tipo_doc == "PASAPORTE") {
    //     $tipoCliente = 2;
    // } else if ($venta->getCliente()->tipo_doc == "CEDULA DE EXTRANJERO") {
    //     $tipoCliente = 3;
    // } else if ($venta->getCliente()->tipo_doc == "CEDULA DE EXTRANJERO") {
    //     $tipoCliente = 3;
    // } else if ($venta->getCliente()->tipo_doc == "SIN NOMBRE") {
    //     $tipoCliente = 5;
    //     $esContribuyente = 0;
    // } else if ($venta->getCliente()->tipo_doc == "DIPLOMATICO") {
    //     $tipoCliente = 4;
    //     $esContribuyente = 0;
    // } else if ($venta->getCliente()->tipo_doc == "CI") {
    //     $tipoCliente = 4;
    //     $esContribuyente = 0;
    // }
    if (in_array($venta->getCliente()->ruc, ['X', 'SIN CEDULA', 'S/C'])) {
        $tipoCliente = 5;
        $esContribuyente = 0;
    } else if (
        in_array($venta->getCliente()->ruc, ['-0'])
    ) {
        $tipoCliente = 4;
        $esContribuyente = 0;
    } else {
        $tipoCliente = 1;
    }
    $credito = CreditoData::getByVentaId($venta->id_venta);
    $cambio = $venta->VerTipoModena()->simbolo == "US$" ? $venta->cambio2 : 1;
    $cdcAsociado = $venta->tipo_venta == 5 ? $venta->cdc_fact : '';
    // 
    $cod = 1;
    $entregas = array();
    $tipo = $venta->metodopago;
    if ($tipo == "Contado") {
        $cajas = CajaDetalle::obtenerVenta($venta->id_venta);
        if ($cajas) {
            foreach ($cajas as $caja) {
                $cod = $caja->CAJA;
                if ($cod == 1) {
                    array_push(
                        $entregas,
                        [
                            'tipo' => 1,
                            'monto' => $caja->IMPORTE,
                            'moneda' => $moneda,
                            'cambio' => $cambio,
                        ]
                    );
                }
                if ($cod == 2) {
                    $cuenta = CuentaBancariaData::getByCajaId($caja->ID);
                    array_push(
                        $entregas,
                        [
                            'tipo' => 5,
                            'monto' => $caja->IMPORTE,
                            'moneda' => $moneda,
                            'cambio' => $cambio,
                        ]
                    );
                }
                if ($cod == 3) {

                    $cuenta = ChequeData::getByCajaId($caja->ID);
                    if ($cuenta) {
                        $banco = BancoData::getBanco($cuenta->id_banco);
                        array_push(
                            $entregas,
                            [
                                'tipo' => 2,
                                'monto' => $caja->IMPORTE,
                                'moneda' => $moneda,
                                'cambio' => $cambio,
                                'infoCheque' => [
                                    "numeroCheque" => $cuenta->nro_cheque,
                                    "banco" => $banco->nombre_banco
                                ],
                            ]
                        );
                    }
                }
                if ($cod == 4) {
                    $cuenta = TarjetaDetalleData::getByCobro($caja->ID);
                    array_push(
                        $entregas,
                        [
                            'tipo' => $cuenta->procesadora_id == 1 ? 3 : 4,
                            'monto' => $caja->IMPORTE,
                            'moneda' => $moneda,
                            'cambio' => $cambio,
                            'infoTarjeta' => [
                                "tipo" => intval($cuenta->tipo),
                                "tipoDescripcion" => "",
                                "titular" => str_replace(".", "", $clienteNombre),
                                "ruc" => $venta->getCliente()->dni,
                                "razonSocial" => $clienteNombre,
                                "medioPago" => 1,
                                "codigoAutorizacion" => 123456789
                            ],
                        ]
                    );
                }
                if ($cod == 6) {
                    array_push(
                        $entregas,
                        [
                            'tipo' => 99,
                            'monto' => $caja->IMPORTE,
                            'moneda' => $moneda,
                            "tipoDescripcion" => "Compensación",
                            'cambio' => $cambio,
                        ]
                    );
                }
            }
        } else {
            array_push(
                $entregas,
                [
                    'tipo' => 1,
                    'monto' => $venta->total - $venta->descuento,
                    'moneda' => $moneda,
                    'cambio' => $cambio,
                ]
            );
        }
    }
    // metodo pago
    $condicionPagos = [
        'tipo' => $tipo == "Contado" ? 1 : 2,
    ];

    if ($tipo == "Contado") {

        $condicionPagos['entregas'] = $entregas;
    } else {
        $infoCuotas = [
            [
                'moneda' => $moneda,
                'monto' => $venta->total - $venta->descuento,
                'vencimiento' => $vencimiento,
            ],
        ];

        $credito = [
            'tipo' => 2,
            'plazo' => "30 días",
            'cuotas' => $cuotas,
            'montoEntrega' => $venta->total - $venta->descuento,
            'infoCuotas' => $infoCuotas,

        ];

        $condicionPagos['credito'] = $credito;
    }
    // 
    $productosItem = array();
    foreach ($operations as $operation) {
        $fila = $operation->is_sqlserver ? "id_sqlserver" : "id_producto";
        $product = ProductoData::getById($operation->producto_id, $fila);
        $precio = floatval($operation->precio);
        $dncpProducto = null;

        if (isset($venta->id_dncp) && $venta->id_dncp != 0) {
            $dncpProducto = array(
                "codigoNivelGeneral" => $product->codigoNivelGeneral,
                "codigoNivelEspecifico" => $product->codigoNivelEspecifico,
                "codigoGtinProducto" => $product->codigoGtinProducto,
                "codigoNivelPaquete" => $product->codigoNivelPaquete
            );
        }
        if ($product->impuesto === 10) {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "10";
        } else if ($product->impuesto == 30) {
            $precioUnitario = number_format(($precio - ((100 * $precio * 70) / (10000 + (5 * 30)))), 6, '.', '');
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "5";
            $arrayIva5 = [
                "precioUnitario" => number_format((((100 * $precio * 70) / (10000 + (5 * 30)))), 6, '.', ''),
                "codigo" => $product->codigo,
                "descripcion" => $product->nombre,
                "observacion" => "",
                "unidadMedida" => $product->presentacion != null ? UnidadesData::getById($product->presentacion)->codigo : null,
                "cantidad" => $operation->q,
                "cambio" => "0",
                "ivaTipo" => 3,
                "ivaBase" => "0",
                "iva" => "0",
                "lote" => "",
                "vencimiento" => "",
                "numeroSerie" => "",
                "numeroPedido" => "",
                "dncp" => $dncpProducto
            ];
            array_push($productosItem, $arrayIva5);
        } else if ($product->impuesto == 5) {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "5";
        } else if ($product->impuesto == 0) {
            $precioUnitario = $precio;
            $ivaTipo = 3;
            $ivaBase = "0";
            $iva = "0";
        } else {
            $precioUnitario = $precio;
            $ivaTipo = 1;
            $ivaBase = "100";
            $iva = "10";
        }

        $array = [
            "precioUnitario" => $precioUnitario,
            "codigo" => $product->codigo,
            "descripcion" => $product->nombre,
            "observacion" => "",
            "unidadMedida" => $product->presentacion != null ? UnidadesData::getById($product->presentacion)->codigo : null,
            "cantidad" => $operation->q,
            "cambio" => "0",
            "ivaTipo" => $ivaTipo,
            "ivaBase" => $ivaBase,
            "iva" => $iva,
            "lote" => "",
            "vencimiento" => "",
            "numeroSerie" => "",
            "numeroPedido" => "",
            "dncp" => $dncpProducto
        ];
        array_push($productosItem, $array);
    }
    $date = new DateTime($vencimiento);
    $fechaVencimiento = $date->format('d/m/Y');
    $dncp = null;
    if (isset($venta->id_dncp) && $venta->id_dncp != 0) {
        $dncpdata = DNCPData::verId($venta->id_dncp);
        $dncp = [
            "modalidad" => $dncpdata->modalidad,
            "entidad" => $dncpdata->entidad,
            "año" => $dncpdata->agno,
            "secuencia" => $dncpdata->secuencia,
            "fecha" => $dncpdata->fecha
        ];
    }
    $array = array(
        "id" => $venta->id_venta,
        "emailEnviado" => $venta->email_enviado,
        "xml" => $venta->xml,
        "factura" => $venta->factura,
        "envio" => $venta->enviado == NULL ? 'No enviado' : $venta->enviado,
        "tipo" => $venta->tipo_venta,
        "fecha" => $venta->fecha,
        "estado" => $venta->estado == 1 ? 'Activo' : 'Anulado',
        "cliente" => $cliente,
        "total" => $venta->total - $venta->descuento,
        "metodoPago" => $venta->metodopago,
        "cambio" => $cambio,
        "moneda" => $venta->VerTipoModena(),
        "credito" => $credito,
        "facturaRemision" => $venta->numero_factura == NULL ? 'SIN RELACION' : $venta->numero_factura,
        "kude" => array(
            "id" => $venta->id_venta,
            "cdc" => $venta->cdc,
            "factura" => $venta->factura,
            "fechaEmision" => $venta->fecha_envio,
            "condicion" => $venta->metodopago == 'Contado' ? $venta->metodopago : $venta->metodopago,
            "rucCliente" => $cliente->dni,
            "cambio" => $cambio,
            "razonCliente" => $clienteNombre,
            "moneda" => $moneda,
            "dirCliente" => $cliente->direccion,
            "telCliente" => $cliente->telefono,
            "cel" => $cliente->celular,
            "iva10" => $venta->iva10,
            "iva5" => $venta->iva5,
            "email" => $cliente->email,
            "vipoVenta" => 1,
            "cdcAsociado" => $cdcAsociado,
            "kudeQr" => $venta->kude,
            "itemsVenta" => json_encode($productos),
            "tipoFactura" => "Factura",
            "operacion" => "Factura Electrónica",
            "docAsociado" => "",
            "remisionNro" => $venta->numero_factura ? $venta->numero_factura : '',
            "vencimiento" => ""
        ),
        "enviar" => array(
            "items" => $productosItem,
            "tipoP" => $cod,
            "tipoOperacion" => $tipoop,

            "condicionPagos" => $condicionPagos,
            "telefonoCliente" => $cliente->telefono,
            "emailCliente" => $cliente->email,
            "id" => $int,
            "descripcion" => "venta",
            "cliente" => "sadsa",
            "telEmisor" => $sucursal->telefono,
            "rucEmisor" => $sucursal->ruc,
            "rucCliente" => $cliente->dni,
            "dirCliente" => $cliente->direccion,
            "telCliente" => $cliente->telefono,
            "factura" => substr($venta->factura, 8),
            "total" => $venta->total - $venta->descuento,
            "moneda" => $moneda,
            "fechaVenta" => date("Y-m-d-h-i", strtotime($venta->fecha)),
            "tipo" => $venta->metodopago,
            "cambio" => $cambio,
            "departamentoCliente" => $cliente->departamento_id,
            "distritoCliente" => $cliente->distrito_id,
            "ciudadCliente" => $cliente->ciudad,
            "cuotas" => $cuotas,
            "establecimiento" => substr($venta->factura, 0, -12),
            "punto" => substr($venta->factura, 4, -8),
            "vencimiento" => $vencimiento,
            "esContribuyente" => $esContribuyente,
            "docCliente" => $tipoCliente,
            "tipoTransaccion" => $venta->transaccion == 0 ? 1 : intval($venta->transaccion),
            "dncp" => $dncp
        )
    );
    array_push($response, $array);
}
header("Content-type:application/json");
$jsdata = json_decode(file_get_contents('php://input'), true);
header("HTTP/1.1 200 OK");
header('Content-Type: text/plain');
echo json_encode([
    "data" => [
        "venta" => $response,
        "insumoId" => $insumoId,
        "sucursal" => $sucursal,

    ],
    "pages" => ceil($paginas[0]->total_registros / 10)
]);
