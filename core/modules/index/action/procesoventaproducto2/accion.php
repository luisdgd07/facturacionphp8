...
if ($c == null) {
	...
	// 					$op->deposito = $c2["deposito"];
	// 					$op->deposito_nombre = $c2["depositotext"];
	// 					$add = $op->registro_producto1_remision();
	...
	if (isset($_POST["cart"])) {
		if (count($cart) > 0) {
			...
			if ($process == true) {
				...
				$sell->configfactura_id = $_POST["configfactura_id"];
				$sell->tipomoneda_id = (int) trim($_POST["idtipomoneda"]);
				...
				$sell->total10 = (float) str_replace(',', '', $_POST["total10"]);
				// $sell->iva10 = $_POST["iva10"];
				$sell->iva10 = (float) str_replace(',', '', $_POST["iva10"]);
				// $sell->total5 = $_POST["total5"];
				$sell->total5 = (float) str_replace(',', '', $_POST["total5"]);
				// $sell->iva5 = $_POST["iva5"];
				$sell->iva5 = (float) str_replace(',', '', $_POST["iva5"]);
				// $sell->exenta = $_POST["exenta"];
				$sell->exenta = (float) str_replace(',', '', $_POST["exenta"]);
				$a1 = $_POST['total'];
				// $a2=(($a1/2000)+1);
				// $sell->total = $a1;
				$sell->total = (float) str_replace(',', '', $a1);
				$sell->n = 1;
				$sell->numerocorraltivo = $j8;
				$a = $_POST["sucursal_id"];
				$sell->sucursal_id = $a;
				...
				$sell->dncp = (int) $_POST["dncp"];

				$sell->vehiculo_id = $_POST["vehiculo_id"];
				$sell->vendedor_id = $_POST["vendedor_id"];
				...
				$sell->cantidaconfigmasiva = $_POST["cantidaconfigmasiva"];
				$sell->tipo_transporte = $_POST['tipo_transporte'];
				$s = 0;
				$sell->cliente_id = $_POST["cliente_id"];
				$s = $sell->venta_producto_cliente4();
