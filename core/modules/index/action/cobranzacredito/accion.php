...
if ($sumatotal <= $sumatotal2) {
	...
	$COBRO_ID = $id->COBRO_ID + 1;

	$registro1->COBRO_ID = $COBRO_ID;
	...
	if ($res == 0) {
		...
		foreach ($pagos as $pago) {
			...
			if ($pago['fecha'] != '') {
				...
				$detalle->usuarion = $_POST["recibon"];
				$d = $detalle->agregar();
			}
			$detalle = new CajaDetalle();
			$detalle->cobroId = $COBRO_ID;
