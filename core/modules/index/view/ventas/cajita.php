...
<script>
    ...
    function anularOperacion(venta) {
        Swal.fire({
        ...
        $.ajax({
            ...
            success: function (dataResult) {
                ...
                for (const [key, venta] of Object.entries(result.data.venta)) {
                        ...
                        `<tr>
                            ...
                            <td>
                                ...
                                <button onclick="anular(${venta.id})" class="btn btn-danger btn-sm btn-flat"><i class='fa fa-trash'></i> </button></abbr>
                                             ...
                                             <td>
                                <abbr title="Anular registro de venta con eliminacion de Operación">
                                <button onclick="anularOperacion(${venta.id})" class="btn btn-secondary btn-sm btn-flat"><i class='fa fa-trash'></i> </button></abbr>
                            </td>
                            <td >
                                <abbr title="Editar">
                                <a class="btn  btn-sm btn-flat" 
                                href="index.php?view=editarpagoventa&id_sucursal=${result.data.sucursal.id_sucursal}&id_venta=${venta.id}">
                    ...
                    if (venta.envio != 'No enviado') {
                        tablab += `<button class="btn btn-primary" onclick='generarTicket(${JSON.stringify(venta.kude)},false)'>Ticket</button>`
                    }
                    tablab +=
                        `</td>
                            <td >`
                    if (venta.envio != 'No enviado') {
                        tablab += `<button class="btn btn-primary" onclick="impresion(${venta.id}, ${result.data.sucursal.id_sucursal})">Descargar</button>`
                    }
                    tablab += `</td>
                            <td >
