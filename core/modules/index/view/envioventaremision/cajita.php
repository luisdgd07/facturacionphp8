...
<script>
    ...
    function buscar() {
        ...
        $.ajax({
            url: `./index.php?action=listarventasremision&cliente=${$("#cliente_id").val()}&desde=${$("#date1").val()}&hasta=${$("#date2").val()}&busqueda=${$("#buscar").val()}&sucursal=<?= $_GET["id_sucursal"] ?>&offset=${pagina * 10}`,
            type: "GET",
            data: {},
            cache: false,
                    ...
                    tablab +=
                        `<tr>
                            <td>
                                ...
                                <button onclick="anularOperacion(${venta.id})" class="btn btn-danger btn-sm btn-flat"><i class='fa fa-trash'></i> </button></abbr>
                            </td>
                            <td >
                                ${venta.id}
                            </td>
