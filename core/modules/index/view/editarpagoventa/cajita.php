...
<script>
    ...
    function actualizarTablacobro() {
        ...
        for (const [id, pago] of Object.entries(pagos)) {
            totalCobrar += parseFloat(pago.monto);
    ...
    function pagar() {
        ...
        // Validación: el monto total pagado debe ser igual al total de la venta (normalizado)
        var totalVentaNormalizado = (moneda === 'DOLARES') ? (total * parseFloat($("#cambio2").val())) : total;
        var totalPagosNormalizado = 0;
        for (const p of pagos) {
            totalPagosNormalizado += parseFloat(p.monto2 || 0);
        }
        if (Math.round(totalPagosNormalizado) !== Math.round(totalVentaNormalizado)) {
            Swal.fire({
                title: "El monto pagado debe ser igual al total",
                text: "Total: " + totalVentaNormalizado.toLocaleString("es-ES") + " - Pagos: " + totalPagosNormalizado.toLocaleString("es-ES"),
                icon: "error",
                confirmButtonText: "Aceptar"
            });
            return;
