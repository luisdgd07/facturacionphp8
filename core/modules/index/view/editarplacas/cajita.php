                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="box">
                            ...
                            <div class="">
                                ...
                                <h2 class="text-center">Serie de las placas:</h2>
                                <label for="inputEmail1" class="col-lg-2 control-label">N° Serie:</label>
                                <div class="col-lg-2 ">
                                    ...
                                    <select name="cliente_id" id="serie" class="form-control">
                                        ...
                                        foreach ($ves as $ve): ?>
                                            <option
                                                value='["<?php echo $ve->registro; ?>","<?php echo $ve->placa_inicio ?>","<?php echo $ve->placa_fin ?>","<?php echo $ve->id_placa ?>"]'>
                                                <?php echo $ve->registro . ' inicio: ' . $ve->placa_inicio . ' fin: ' . $ve->placa_fin . " cantidad:" . ($ve->total_placas - $ve->diferencia) ?>
                                            </option>
                                        <?php endforeach;
                                        ?>
                                    </select>
                  ...
                  <div class="col-lg-2">
                    <input type="number" class="form-control" id="finp" placeholder="N Fin:">
                                ...
                                <div class="col-lg-2">
                                    <input type="number" class="form-control" id="cantidadPl">
                                ...
                                <div class="col-lg-offset-2">
                                    <div class="checkbox">
                                        <label>
                                            ...
                                            <button id="accion" class="btn btn-lg btn-success"
                                                onclick="accion()"><b></b>Editar</button></label>
                                        <br>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <input disabled class="form-control" type="text" id="serieE" placeholder="Precio">
                        <br>

                        <label for="">Nro Inicial</label>
                        <input class="form-control" type="number" id="iniE" placeholder="Precio">
                        <br>

                        <label for="">Nro Final</label>
                        <input class="form-control" type="number" id="finE" placeholder="Precio">
                        <br>

                        <label for="">Cantidad</label>
                        <input type="number" class="form-control" id="cantidadE" placeholder="Cantidad">


                    </div>
                    <div class="modal-footer">
                        <button type="button" data-bs-dismiss="modal" class="btn btn-danger" onclick="dismiss()">Cerrar</button>
                        <button type="button" class="btn btn-primary" onclick="editplaca()">editar</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function agregarPlaca() {
        ...
        if (placas.some(item => item.id === pl[3])) {
            Swal.fire({
                title: "Ya posee esta placa agregada",
                icon: 'error',
            ...
            placas.push({
                ini: pl[1],
                fin: parseInt(pl[1]) + parseInt($("#cantidadPl").val()),
    ...
    //       //   $("#serie").html("");
    //       $.ajax({
    //           url: 'index.php?action=obtenerplaca',
    //           type: 'GET',
    //           data: {
    ...
    //           dataType: 'json',
    //           success: function(json) {
    //               console.log('dsfdsfds', json)
    //               for (var i = 0; i < json.length; i++) {
    //                   select += `<option value="${json[i].id_placas}">${json[i].registro}</option> `
    ...
    function actualizarTablaPlacas() {
        ...
        for (const [id, pago] of Object.entries(placas)) {
            tabla += `<tr><td> ${pago.serie}</td><td> ${pago.ini}</td><td> ${pago.fin}</td><td> ${pago.cantidad}</td> <td> <button class="btn btn-warning" onclick="editarplaca('${pago.serie}',${pago.ini}, ${pago.fin},${pago.cantidad},${id},${pago.id})">Editar</button><button class="btn btn-danger" onclick="eliminarplaca(${id})">Eliminar</button></td></tr>`;
        }
        $("#tbody").html(tabla);
    ...
    function editarplaca(serie, ini, fin, cantidad, id, id2) {
        console.log(fin);
        placasEdita = id2;
        edita = id
        $("#serieE").val(serie);
