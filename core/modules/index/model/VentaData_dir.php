...
class VentaData_dir
{
	...
	public static function getbyfactura($codigo, $id_sucursal)
	{
		...
		return Model::one($query[0], new VentaData_dir());
	}
	...
	{
		$sql = "select * from " . self::$tablename . " where factura=\"$codigo\" and sucursal_id=$id_sucursal and tipo_venta=4 ";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
		while ($r = $query[0]->fetch_array()) {
	...
	public static function versucursalventas($id_sucursal)
	{
		...
		$query = Executor::doit($sql);
		return Model::many($query[0], new VentaData_dir());
	}
	...
	{
		$sql = "select * from " . self::$tablename . " where sucursal_id=$id_sucursal and accion_id=2 and ( tipo_venta=4) and estado =1 and fecha >= '2023-01-01' and  tipo_remision != 3 order by id_venta desc";
		$query = Executor::doit($sql);
		return Model::many($query[0], new VentaData_dir());
	}
	...
	{
		$sql = "insert into " . self::$tablename . " (presupuesto,factura,configfactura_id,tipomoneda_id,cambio,cambio2,simbolo2,formapago,codigo,fechapago,metodopago,total10,iva10,total5,iva5,exenta,total,n,numerocorraltivo,sucursal_id,cliente_id,usuario_id,cantidaconfigmasiva,accion_id,fecha,REMISION_ID, estado, cdc_fact, numero_factura, tipo_venta) ";
		$sql .= "value ($this->presupuesto,\"$this->factura\",$this->configfactura_id,$this->tipomoneda_id,\"$this->cambio\",$this->cambio2,\"$this->simbolo2\",\"$this->formapago\",$this->codigo,\"$this->fechapago\",\"$this->metodopago\",\"$this->total10\",\"$this->iva10\",\"$this->total5\",\"$this->iva5\",\"$this->exenta\",\"$this->total\",$this->n,\"$this->numerocorraltivo\",$this->sucursal_id,$this->cliente_id,$this->usuario_id,\"$this->cantidaconfigmasiva\",2,\"$this->fecha\",$this->REMISION_ID,1,\"$this->cdc_fact\",\"$this->num_fact\",5)";
		return Executor::doit($sql);
	}
	public function venta_producto_cliente_exportacion()
	{
		$sql = "insert into " . self::$tablename . " (presupuesto,factura,configfactura_id,tipomoneda_id,cambio,cambio2,simbolo2,formapago,codigo,fechapago,metodopago,total10,iva10,total5,iva5,exenta,total,n,numerocorraltivo,sucursal_id,cliente_id,usuario_id,cantidaconfigmasiva,accion_id,fecha,REMISION_ID, estado, cdc_fact, numero_factura, tipo_venta,agente_id, fletera_id, id_chofer, condiNego, manifiesto) ";
		$sql .= "value ($this->presupuesto,\"$this->factura\",$this->configfactura_id,$this->tipomoneda_id,\"$this->cambio\",$this->cambio2,\"$this->simbolo2\",\"$this->formapago\",$this->codigo,\"$this->fechapago\",\"$this->metodopago\",\"$this->total10\",\"$this->iva10\",\"$this->total5\",\"$this->iva5\",\"$this->exenta\",\"$this->total\",$this->n,\"$this->numerocorraltivo\",$this->sucursal_id,$this->cliente_id,$this->usuario_id,\"$this->cantidaconfigmasiva\",2,\"$this->fecha\",$this->REMISION_ID,1,'','',20,\"$this->agente\" ,\"$this->fletera\",\"$this->chofer_id\",\"$this->condiNego\",\"$this->manifiesto\")";
		return Executor::doit($sql);
	}
    ...
    */
   	public function venta_producto_cliente4()
	{
		$sql = "insert into " . self::$tablename . " (presupuesto,factura,configfactura_id,tipomoneda_id,cambio,cambio2,simbolo2,formapago,codigo,fechapago,metodopago,total10,iva10,total5,iva5,exenta,total,n,numerocorraltivo,sucursal_id,cliente_id,usuario_id,cantidaconfigmasiva,accion_id,fecha,REMISION_ID, tipo_venta, estado,id_chofer, id_vehiculo, id_dep, id_ciudad, destino,tipo_remision) ";
		$sql .= "value ($this->presupuesto,\"$this->factura\",$this->configfactura_id,$this->tipomoneda_id,\"$this->cambio\",$this->cambio2,\"$this->simbolo2\",\"$this->formapago\",$this->codigo,\"$this->fechapago\",\"$this->metodopago\",\"$this->total10\",\"$this->iva10\",\"$this->total5\",\"$this->iva5\",\"$this->exenta\",\"$this->total\",$this->n,\"$this->numerocorraltivo\",$this->sucursal_id,$this->cliente_id,$this->usuario_id,\"$this->cantidaconfigmasiva\",2,\"$this->fecha\",1,4,1, \"$this->chofer_id\", \"$this->vehiculo_id\", \"$this->dep_id\", \"$this->ciudad_id\",\"$this->destino\",\"$this->tipo_remision\")";
		return Executor::doit($sql);
	}
	...
	{
		$sql = "update " . self::$tablename . " set estado=0  where id_venta=$this->REMISION_ID";
		return Executor::doit($sql);
	}
	...
	public function actualizarFE($id, $envio, $xml, $cdc, $kude, $fecha)
	{
		$sql = "UPDATE `venta` SET `enviado` = '$envio', `cdc` = '$cdc' , `xml` = '$xml' , `kude` = '$kude', `fecha_envio` = '$fecha' WHERE `venta`.`id_venta` = $id;";
		Executor::doit($sql);
	}
	...
	public static function getAllPersonalizado($start, $end, $id_sucursal, $enviado, $tipo, $cliente)
	{
		$sql = "";
		if ($start == $end) {
			$sql = "select * from " . self::$tablename . " where sucursal_id=$id_sucursal and date(fecha) = \"$start\"  ";
			// 	$sql = "select * from " . self::$tablename . " where date(fecha) = \"$start\"  and sucursal_id=$id_sucursal   and tipo_venta=0 and enviado like \"$enviado\" order by fecha desc";

		} else {
			$sql = "select * from " . self::$tablename . " where sucursal_id=$id_sucursal and date(fecha) >= \"$start\" and date(fecha) <= \"$end\"";
		}
		if ($enviado != "todos") {
