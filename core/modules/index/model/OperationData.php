...
class OperationData
{
	...
	public function registro_producto1_remision()
	{
		$sql = "insert into remision_detalle (sucursal_id,precio,producto_id,q,accion_id,venta_id,fecha, motivo, stock_trans, precio1,deposito,deposito_nombre) ";
		$sql .= "value (\"$this->sucursal_id\",\"$this->precio\",\"$this->producto_id\",\"$this->q\",$this->accion_id,$this->venta_id,\"$this->fecha\",\"$this->motivo\",$this->stock_trans,$this->precio1,$this->deposito, \"$this->deposito_nombre\")";
		return Executor::doit($sql);
	}
	public function registro_producto1_nota()
	{
		$sql = "insert into nota_de_credito_venta_detalle (sucursal_id,precio,producto_id,q,accion_id,venta_id,fecha, motivo, stock_trans, precio1,deposito,deposito_nombre) ";
		$sql .= "value (\"$this->sucursal_id\",\"$this->precio\",\"$this->producto_id\",\"$this->q\",$this->accion_id,$this->venta_id,\"$this->fecha\",\"$this->motivo\",$this->stock_trans,$this->precio1,$this->deposito, \"$this->deposito_nombre\")";
		return Executor::doit($sql);
	}
	...
	public static function getOperacionProductos($venta_id)
	{
		$sql = "SELECT * 
		FROM `operacion` 
		INNER JOIN `producto` AS prod ON `prod`.`id_producto`= `operacion`.`producto_id`
		INNER JOIN `unidades` AS unidad ON `unidad`.`id`= prod.`presentacion`
	...
	{
		$sql = "DELETE FROM operacion WHERE `operacion`.`venta_id` = $id";
		return Executor::doit($sql);
		// return $sql;
	}
}
