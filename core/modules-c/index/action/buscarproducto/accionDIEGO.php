...
<?php 
  ...
  <tr class="<?php if($q<=$product->minimo_inventario){ echo "danger"; }?>">
    ...
    <td style="width:250px;"><form method="post" action="index.php?action=agregarcantidad">
...
<?php if(isset($_GET["producto"]) && $_GET["producto"]!=""):?>
  <?php
$products = ProductoData::getLike($_GET["producto"]);
$proceso1 = OperationData::getLike($_GET["producto"]);
...
<table class="table table-bordered table-hover">
  ...
  <tr class="<?php if($q<=$product->minimo_inventario){ echo "danger"; }?>">
    ...
    <td style="width:300px;"><form method="post" action="index.php?action=agregarcantidad1">
    ...
    <div class="input-group">
        ...
        if (count($extraerdata)>0) {
          foreach ($extraerdata as $data) { ?>
              <?php  $data->IMPORTE; 
			  $datito = ProductoData::vertipomonedadescrip2($sucursales->id_sucursal, $data->PRECIO_ID);
              ?>
          <?php }
        }?>
      ...
      <input type="number" class="form-control" required name="q" placeholder="Cantidad ..." min="1">
	  <input type="hidden" name="precio" value="<?php echo $data->IMPORTE; ?>">
        <span class="input-group-btn">
      <button type="submit" class="btn btn-warning"><i class="glyphicon glyphicon-plus-sign"></i> Agregar</button>
        </span>
    </div>
...
<?php if(count($proceso1)>0){
  ?>
<h3>Resultados de la Búsqueda</h3>
<form method="post" action="index.php?action=agregarcantidad3">
<table class="table table-bordered table-hover">
  <thead>
    <th>#</th>
    <th>Código</th>
  ...
  <?php
   foreach($proceso1 as $process):
  ?>
  <tr>
    <td><?= $process->id_proceso; ?></td>
    <td style="width:80px;"><?php echo $process->getProducto()->codigo; ?></td>
    <td><?php echo $process->getProducto()->nombre; ?></td>
    
    <td style="width:300px;">
    <input type="hidden" name="id_sucursal[]" id="id_sucursal" value="<?php echo $sucursales->id_sucursal; ?>">
    <input type="hidden" name="tid" id="id_sucursal" value="<?php echo $sucursales->id_sucursal; ?>">
    <input type="hidden" name="stock[]" value="<?= $q; ?>">
    <input type="hidden" name="tipoproducto[]" value="<?php echo $process->getProducto()->tipoproducto()->TIPO_PRODUCTO; ?>">
    <input type="hidden" name="producto_id[]" value="<?php echo $process->producto_id; ?>">
  <input type="hidden" name="precios[]" value="<?= $process->precio1;?>">
    <div class="input-group">
      <input type="hidden" name="precio[]" value="<?= $process->precio;?>">
      <input type="number" class="form-control" required name="q[]" value="<?= $process->q; ?>" placeholder="Cantidad ..." min="1" readonly="readonly">
      <input type="hidden" name="cli[]" value="<?= $process->venta_id; ?>">
       </td>
  </tr>
...
<span class="input-group-btn">
      <button type="submit" class="btn btn-success"><i class="glyphicon glyphicon-plus-sign"></i> Validar</button>
        </span>
    </div>
    </form>
  <?php
