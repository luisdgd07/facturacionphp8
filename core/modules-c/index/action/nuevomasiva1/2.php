...
$cantidad_fact = round(($_POST['cantidadd']/(2000/$_POST['precio'])), 0, PHP_ROUND_HALF_DOWN);




for ($i = 1; $i <= $cantidad_fact; $i++) {
    $masiva = new VentaData1();
    foreach ($_POST as $k => $v) {
      $masiva->$k = $v;
      # code...
  ...
  $facto=round((2000/$a3), 0, PHP_ROUND_HALF_DOWN);
  ...
  $suma_facturas = $masiva->montofactura=$facto*$a3 + $suma_facturas;
  
  $suma_cantidad = $facto + $suma_cantidad;
...
$diferencia_cantidad=$a20 - $suma_cantidad;

$a3=$_POST['precio'];


//aqui hago el ultimo registro de la venta masiva

if ($control_final ==0 and $cantidad_fact == $i) {
