...
class OperationData
{
	...
	{
		$sql = "select * from " . self::$tablename . " where transaccion=1 and sucursal_id=$id_sucursal OR( transaccion=0 AND accion_id=1 and sucursal_id=$id_sucursal) or ( transaccion=0 AND accion_id=2 and sucursal_id=$id_sucursal)  ORDER BY venta_id desc ";
		$query = Executor::doit($sql);
		return Model::many($query[0], new OperationData());
	}
	...
	{
		$sql = "select * from " . self::$tablename . " where sucursal_id=$id_sucursal and accion_id=2 and transaccion =0 order by fecha desc";
		$query = Executor::doit($sql);
		return Model::many($query[0], new OperationData());
	}
